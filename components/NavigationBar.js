...
function NavigationBar({ card }) {
  ...
  const nonActiveElement =
    "flex gap-4 py-3 my-2 hover:bg-socialBlue hover:bg-opacity-20  -mx-10 px-10 rounded-md transition-all hover:scale-90 hover:shadow-md hover:shadow-gray-300 hover:text-black";
  return (
    <Card className={card}>
      <div className="px-4">
        <h2 className="text-gray-400 font-bold mb-3">Navigation</h2>
        <Link
          className={router.pathname === "/" ? activeElement : nonActiveElement}
          href="/"
        >
          <svg
